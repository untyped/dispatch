...
@itemize{
  @item{simple configuration of URLs and controller procedures;}
  @item{mechanisms for separating view and controller code despite the unidirectionality of @scheme[require] statements;}
...
This means you can place your @scheme[define-site] expression in a central module in your web application and require it into all your view and controller code. Because @scheme[define-controller] doesn't actually @scheme[define] any new identifiers, your entire site is available to all the Scheme code in your application.

@subsection{Access control (new in Dispatch 3)}
...
Dispatch runs the @scheme[#:access?] expression before it calls the main body of the controller. If the expression returns @scheme[#f], the user is redirected to a standard ``access denied'' page.

@subsection{Hyperlink generation}
...
@bold{TODO: Still writing this manual... got to here so far.}
