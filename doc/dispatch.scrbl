...
@(require (for-label (file "../dispatch.ss")))
...
This section of this document provides a brief overview of the features of Dispatch. The @seclink["quick"]{next section} provides a working example of how to set up a simple blog from scratch using Dispatch. The @seclink["api"]{third section} provides a reference for the Dispatch API.
...
This section provides a worked example of using Dispatch to set up the blog from the @secref{intro}. The example also uses Instaservlet to simplify the web server configuation. Some details are skipped over here: see the @secref["api"] for more information on the macros and procedures used.
...
In this code, the @scheme[index] controller is generating a list of posts using a helper procedure called @scheme[index-item-html]. @scheme[index-item-html] is using a Dispatch API procedure called @scheme[controller-url] to create URLs that point to @scheme[review-post]. @scheme[controller-url] takes at its arguments the controller to link and the values of any URL pattern arguments: note that there is no @italic{request} argument.

Note that @scheme[review-post] is begin provided from the @scheme[define-site] statement @scheme{site.ss}, not from the @scheme[define-controller] statement in the local module. We can easily move @scheme[index-item-html] out into a separate module of view code without creating a cyclic module dependency. For the moment, however, we just need to see the code working. Re-run the application and go to @scheme{http://localhost:8765/} in your browser.
...
@defmodule[(planet "dispatch.ss" ("untyped" "dispatch.plt" 3)) #:use-sources ((file "dispatch.ss"))]
...
Literal strings in the pattern are passed through @scheme[pregexp-quote] to remove the special meanings of any reserved characters. Args are converted to fragments using their @scheme[pattern] fields, which are wrapped in parentheses to enable regular expression capture:
...
@defproc[(controller-pipeline [controller controller?]) (listof stage?)]{
